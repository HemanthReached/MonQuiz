...
class SplashScreen1 : BaseActivity() {
    ...
    private fun startHandler() {
        handler = Handler()
        runnable = Runnable {
            val intentMove: Intent = if(!PrefsHelper().getPref(OwlizConstants.login,false)){
                Intent(this, SignUpActivity::class.java)
            } else if(!PrefsHelper().getPref(OwlizConstants.username,false)){
